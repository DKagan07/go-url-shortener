package store

import (
	"testing"

	"github.com/stretchr/testify/assert"
)

var testStorageService = &StorageService{}

func init() {
	testStorageService = InitializeStore()
}

func TestStorageInit(t *testing.T) {
	assert.True(t, testStorageService != nil)
}

func TestInsertionAndRetreival(t *testing.T) {
	initialLink := "https://www.guru3d.com/news-story/spotted-ryzen-threadripper-pro-3995wx-processor-with-8-channel-ddr4,2.html"
	userUUId := "e0dba740-fc4b-4977-872c-d360239e6b1a"
	shortURL := "Jsz4k57oAX"

	ss := InitializeStore()

	ss.SaveUrlMapping(shortURL, initialLink, userUUId)

	retreivedUrl := ss.RetreiveInitialUrl(shortURL)
	assert.Equal(t, initialLink, retreivedUrl)
}
